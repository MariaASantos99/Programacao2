...
public class MainTrain {
  public static void main(String[] args) {
    ...
    if (trainCapacity < wagonCapacity) {
      err.printf("ERRO: A capacidade do comboio deve superar a de um vagao!\n");
      exit(1);
    }

    // Cria comboio com as capacidades indicadas:
    Train train = new Train(wagonCapacity, trainCapacity);
    out.printf("(Capacidade dos vagoes: %.1f ton.)\n", wagonCapacity);
    ...
    for (int i = 2; i < args.length; i++) {
      ...
      if (args[i].equalsIgnoreCase("R")) {  // "R" => retirar um vagão
        ...
        out.printf("Retira vagao com %.1f ton\n", c);
        ...
        if (c >= 0) {  // valor positivo => acrescentar um vagão com essa carga
          if (c > wagonCapacity) {  // se carga supera capacidade de um vagão,
            err.printf("ERRO: Sobrecarga de vagao!\n");
            exit(1);
          }
          ...
          out.printf("Junta vagao com %.1f ton\n", c);
          ...
          out.printf("Descarrega %.1f ton e retira %d vagoes vazios.\n", c, n);
        }
      } else {
        err.printf("ERRO: Argumento invalido!\n");
        exit(1);
      }

      // Mostra o estado do comboio: tamanho, carga total e lista dos vagões
      out.printf("  (%d vagoes, %.1f ton):\tLocO",
